...
export async function getUser (ctx: RouterContext): Promise<void> {
  ...
  return await fetchAPI(`users${query}`, ctx.state.user.access_token).then(data => {
    ctx.body = formatUser(data.data.pop())
  }).catch(err => error(ctx, err, 400, 'Failed to get infos'))
}
...
export async function getFollows (ctx: RouterContext): Promise<void> {
  ...
  return await fetchAPI(`users/follows${query}`, ctx.state.user.access_token).then(async data => {
  ...
  }).catch(err => error(ctx, err, 400, 'Failed to get follows'))
}
...
export async function getUserVideos (ctx: RouterContext): Promise<void> {
  ...
  return await fetchAPI(`users${query}`, ctx.state.user.access_token).then(async data => {
    const first = Number(ctx.query.first)
    query = objectToQuery({
    ...
    return await fetchAPI(`videos${query}`, ctx.state.user.access_token).then(data => {
      ctx.body = {
        videos: data.data.map((video: Video) => formatVideo(video)).filter((video: Video) => video.thumbnail_url),
        next: data.pagination.cursor
      }
    }).catch(err => error(ctx, err, 400, 'Failed to get user videos'))
  }).catch(err => { throw err })
}
...
export async function getVideos (ctx: RouterContext): Promise<void> {
  ...
  }).catch(err => error(ctx, err, 400, 'Failed to get videos'))
}
